...
for(item in str_list[1:3]){
  ...
  data_final <- rbind(data_final,cbind.data.frame(name=item,positive=length(which(polarities == "positive")),negative=length(which(polarities == "negative")),neutral=length(which(polarities == "neutral"))))
